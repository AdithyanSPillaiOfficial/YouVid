...
function VideoView({videoUrl, isVideo, handleFormatOpen, formatOpen}) {
  ...
  return (
    <div className='relative flex flex-col rounded-lg bg-white shadow-sm border border-slate-200 p-5 box-border'>
        {isVideo ? (<video controls autoPlay src={videoUrl} onLoadedData={() => setIsLoading(!isLoading)} />) : (<audio controls autoPlay className='w-full' onLoadedData={() => setIsLoading(!isLoading)}><source src={videoUrl} type="audio/mpeg" /></audio>)}
